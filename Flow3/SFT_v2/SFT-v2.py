...
wandb.init(project="statistical-llm-sft", name="SFT-v2")
...
def apply_template2(examples):
    ...
    for problem, step in zip(problems, steps):
        ...
        temp.append({"role": "assistant", "content": "<reasoning>\n" + step + "\n</reasoning>\n"})
        messages.append(temp)
...
dataset2 = dataset2.map(apply_template2, batched=True, remove_columns=["problem", "steps"])


####################################### 3. stat. CoT + QA  
def apply_template3(examples):
...
dataset3 = dataset3.map(apply_template3, batched=True, remove_columns=["question", "reasoning", "answer"])
...
dataset4 = dataset4.shuffle().select(range(30000))
